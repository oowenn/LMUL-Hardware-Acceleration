...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            #there is a scalar multiplication here but LMUL is used here as a matrix multiplier replacement
            att = lmul_matmul(q, k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
            att = att.masked_fill(self.bias[:,:,:T,:T] == 0, float('-inf'))
            att = F.softmax(att, dim=-1)
            att = self.attn_dropout(att)
            y = lmul_matmul(att, v) # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
        else:
