...
typedef struct {
    ...
    XQwen_superblock_uio_map maps[ MAX_UIO_MAPS ];
} XQwen_superblock_uio_info;

/***************** Variable Definitions **************************************/
static XQwen_superblock_uio_info uio_info;
...
static int uio_info_read_name(XQwen_superblock_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XQwen_superblock_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XQwen_superblock_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XQwen_superblock_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XQwen_superblock_Initialize(XQwen_superblock *InstancePtr, const char* InstanceName) {
	XQwen_superblock_uio_info *InfoPtr = &uio_info;
